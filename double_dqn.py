...
if __name__ == '__main__':
    ...
    print(device)
    agent = DQN(input_size, action_dim, lr, gamma, epsilon, target_update, buffer_size, device, "DoubleDQN")

    #load_net = int(input("load or not(if this is your first time to run, input 0, else 1):"))
    #if load_net == 1:
    agent.q_net = torch.load(r'E:\上课\RL\atari\model.pkl')
    agent.target_q_net = torch.load(r'E:\上课\RL\atari\model.pkl')
